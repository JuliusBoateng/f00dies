...
/* uodate */
if ($stmt = mysqli_prepare($link, "UPDATE menuitems SET $update_select = ? WHERE name = ? and rest_name = ?")) {
	mysqli_stmt_bind_param($stmt, 'dss', $_GET['update_input'],$_GET['name'],$_GET['location']);
  mysqli_stmt_execute($stmt);

  if (mysqli_affected_rows($stmt)){
